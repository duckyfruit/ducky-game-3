...
Load< LitColorTextureProgram > lit_color_texture_program(LoadTagEarly, []() -> LitColorTextureProgram const * {
	...
	lit_color_texture_program_pipeline.flatprogram = ret->flatprogram;
...
LitColorTextureProgram::LitColorTextureProgram() {
	//Compile vertex and fragment shaders using the convenient 'gl_compile_program' helper function:
		...
		flatprogram = gl_compile_program(
			//vertex shader:
			"#version 330\n"
			"uniform mat4 OBJECT_TO_CLIP;\n"
			"uniform mat4x3 OBJECT_TO_LIGHT;\n"
		...
		GLuint TEX_sampler2D2 = glGetUniformLocation(flatprogram, "TEX");

		//set TEX to always refer to texture binding zero:
		glUseProgram(flatprogram); //bind program -- glUniform* calls refer to this program now

		glUniform1i(TEX_sampler2D2, 0); //set TEX to sample from GL_TEXTURE0
